...
const Connection = ({connection, updateConnection, channel, updateChannel})=>{
    ...
    useEffect(()=>{
        webSocket.current = new WebSocket('ws://excelsior-signalling-server.herokuapp.com/');
    ...
    const onOffer = ({ offer, name }) => {
        ...
        connection.setRemoteDescription(new RTCSessionDescription(offer)).then(()=> connection.createAnswer())
        .then(answer => connection.setLocalDescription(answer)).then(()=>
        send({ type: 'answer', answer: connection.localDescription, name})).catch(e=>{
            console.log({e});
        });
    };
    ...
    const downloadFile = (bytes, name) => {
        ...
        writeFile(path, bytes).then((success) => {
